...
function storyArchitecturePrompt(bundle, sourceAudit) {
  ...
  return [
    ...
    '- 用户可见字段不得显示制作/治理备注，例如“脱敏模拟数据”“不代表真实”“外发前替换真实授权数据”“仅用于测试/生成/演示”。如需保留，只能写入 missing_info、commercial_risks、source_trace 或 material_hygiene 等内部字段。',
...
function extractionPrompt(bundle, sourceAudit, storyPlan, clarifications) {
  ...
  return [
    ...
    '- proof_object 只作为内部路由 id；用户可见标题、副标题、核心卡片标题和 note 优先写入 display_copy，不要把 proof_object、page family、layout variant、Pareto/帕累托、responsibility loop 等内部或统计术语写进 display_copy。',
    '- 每个 claim 必须输出 business_domain、chain_stage、depth_domain、industry_objects、proof_intent；这些字段驱动行业 depth preflight 和 route selection，不能只靠关键词或 proof_object 名称。',
    '- 不能只抽指标页；每个行业 deck 必须覆盖行业对象、经营链路、证明对象和行动闭环。缺数据、缺图片、缺截图、缺证书或缺客户授权时写 missing_info / asset_requirements / clarification_candidates，不要静默丢弃该深度域。',
    '- 消费零售/跨境电商至少识别产品/SKU、平台/渠道、消费者反馈、活动漏斗、履约物流、利润现金、行动闭环、品牌/视觉证据；品牌/视觉证据可以由真实图片、产品结构、SKU 角色、消费者原话或明确的信息缺口承接。',
    ...
    '- 用户可见标题、页脚、标签、caption、note、结尾和组件文案不得包含“脱敏模拟数据”“不代表真实”“外发前替换真实授权数据”“仅用于测试/生成/演示”等制作/治理备注；这些信息只能进入 missing_info、commercial_risks、source_trace 或 material_hygiene 等内部字段。',
    ...
    '- 客户声音/调研/评论/反馈类页，proof_object 可以选择 issue-frequency-ranking 或 review-sentiment-ranking，但 display_copy.title 不要直接出现 Pareto/帕累托。',
    '',
    'Stage 1 来源审计：',
    '',
    sourceAudit ? jsonBlock(sourceAudit) : '```json\n{ "paste": "material-source-audit/v1 output here" }\n```',
...
function criticPrompt(bundle, extraction, qa) {
  return [
    ...
    '- 用户可见 copy 是否泄漏内部术语，如 proof object、page family、layout variant、downtime-pareto、responsibility-loop、DOWNTIME PARETO、RESPONSIBILITY LOOP。',
    '- 闭环/治理页是否跨行业复用同一套“责任分工/问题·责任·动作/责任不落空”文案，而不是按制造、医疗、SaaS 语义分别成立。',
    '- 封面与尾页是否过度同骨架复用，导致跨行业 deck 几乎一个样子。',
    '- proof_object 与 display_copy 标题是否语义冲突，例如内部是 loss-pareto，但可见标题写成客户顾虑排序，或反过来。',
