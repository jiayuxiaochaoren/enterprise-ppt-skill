    ...
    claim_spine: [
      {
        ...
        proof_object: 'loss-pareto | issue-frequency-ranking | review-sentiment-ranking | valuation-sensitivity | quality-handoff | member-cohort-ladder | channel-efficiency-matrix | monthly-pulse-trend | waterfall-bridge | dispatch-map | adoption-funnel | service-blueprint | platform-capability-map | production-topology | finance-bridge | portfolio-table | risk-matrix | manufacturing-action-loop | healthcare-quality-loop | saas-governance-loop | generic-action-loop | lookbook | case-comparison | metric-board | report-board',
        display_copy: {
          title: 'primary visible page title for users',
          subtitle: 'primary visible subtitle/supporting sentence for users',
          core_title: 'visible core card title or chart-side title',
          core_body: 'visible core card body or chart-side explanation',
          kicker: 'visible small section label or page kicker',
          note: 'visible footnote/note when explicitly needed'
        },
        evidence_ids: ['ev-001'],
        source_ids: ['src-001'],
  ...
  return {
    ...
    matchIndustryIds: [...ids],
	    industryPack: pack ? {
	      ...
	      coverArchetype: pack.coverArchetype,
	      dividerArchetype: pack.dividerArchetype,
	      bodyLayoutPool: pack.bodyLayoutPool,
	      closingArchetype: pack.closingArchetype,
	      paletteTokenSet: pack.paletteTokenSet,
	      textureBackgroundPolicy: pack.textureBackgroundPolicy,
...
function buildModelPrompt(bundle = {}) {
  ...
  return [
    ...
    '- proof_object 是内部路由 id，应优先选择行业专用对象；不确定时用 report-board 或 metric-board。',
    '- display_copy 是用户可见文案入口；title/subtitle/core_title/core_body/kicker/note 尽量填写，且不要把内部 route 名、proof object、page family、layout variant、Pareto/帕累托、responsibility loop、downtime-pareto 等内部或统计术语直接写进可见标题。',
    ...
    '- claim/support/bullets/note/display_copy.* 必须是客户可见的正式文案，不要写“材料显示、材料中、原材料、企业 PDF、PDF 简介口径、该页用于、正式交付前建议、图册页优先、适合某类材料、模型抽取”等制作备注。',
    '- 对客户声音/调研/评论/反馈类材料，优先使用“顾虑排序 / 问题排序 / 主题排序”等中文表达，不要把 Pareto/帕累托 直接放进用户可见标题。',
