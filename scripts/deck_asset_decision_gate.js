...
function buildGate(planPath, answersPath = '') {
  ...
  const ctx = makeDeckContext(rawPlan);
  const questions = [];
...
try {
  ...
  const gate = buildGate(opts.plan, opts.answers);
  writeJson(opts.out, gate);
  if (opts.outPlan && gate.resolvedPlan) writeJson(opts.outPlan, gate.resolvedPlan);
