...
function createOverlayNativeEvidence(deps = {}) {
  ...
  function nativeDrawnEvidenceFor(plan = {}, s = {}, componentId = '', contract = {}, slide = null) {
    ...
    if (componentId === 'workflow-rail' && hasWorkflow) return evidence([/workflow|process|architecture|flow|content|stage|native/i], Math.max(1, (s.workflow || s.workflows || s.automationWorkflow || s.steps || s.phases || s.items || s.platformCapabilities || []).length || 1), 'native SaaS renderer draws workflow implementation rail');
