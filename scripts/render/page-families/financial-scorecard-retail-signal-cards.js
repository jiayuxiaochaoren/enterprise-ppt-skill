function createRetailSignalCardsRenderer(ctx = {}) {
  ...
  function drawRetailSignalCards(slide, basket, conversion, right) {
    [basket, conversion].forEach((m,i)=>{
      ...
      addLabel(slide, i===0 ? '商品信号' : '履约信号', { x:right.x+0.18, y:y+0.16, w:1.10, h:0.10, fontSize:6.8, color, charSpace:0 });
      addText(slide, m.label || (i===0 ? '客单价' : '门店转化'), { x:right.x+0.18, y:y+0.40, w:1.18, h:0.16, fontSize:8.9, bold:true, color:C.text, fit:'shrink', valign:'mid' });
      addNumber(slide, m.value || '—', { x:right.x+1.60, y:y+0.32, w:0.76, h:0.24, fontSize:13.6, color, align:'right', fit:'shrink' });
    });
  }
