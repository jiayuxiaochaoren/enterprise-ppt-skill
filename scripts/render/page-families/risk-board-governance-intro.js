function createRiskBoardGovernanceIntro(ctx = {}) {
  ...
  function drawGovernanceEditorialCore(slide, s, intro) {
    ...
    addLabel(slide, 'EDITORIAL CORE', {
      x:intro.x+0.28, y:intro.y+0.34, w:1.24, h:0.10, fontSize:5.8, color:C.accent, charSpace:0.8
    });
    ...
    addText(slide, s.coreBody || s.note || '每个治理动作都需要责任人、节奏、证据和决策去向。', {
    ...
    addLabel(slide, 'OWNER · CADENCE · EVIDENCE · DECISION', {
      x:intro.x+0.28, y:intro.y+3.14, w:1.56, h:0.16,
      fontSize:5.5, color:'64748B', charSpace:0.45, fit:'shrink'
    });
  }
