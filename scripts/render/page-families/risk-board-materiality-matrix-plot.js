function createRiskBoardMaterialityMatrixPlot(ctx = {}) {
  ...
  function drawMaterialityMatrixPlot(slide, plan, s, rows, axes, matrix) {
    ...
    addText(slide, governmentMateriality ? '年度推进区' : (generalOperationsMateriality ? '优先推进区' : '优先治理区'), {
    ...
    addText(slide, axes.y || (governmentMateriality ? '政策影响' : (generalOperationsMateriality ? '相关方关注' : '利益相关方影响')), {
      x:matrix.x+0.02, y:matrix.y+0.34, w:0.50, h:0.52,
      fontSize:6.8, color:C.muted, rotate:270, fit:'shrink'
    });
    addText(slide, axes.x || (governmentMateriality ? '经营影响' : (generalOperationsMateriality ? '经营影响' : '业务影响')), {
