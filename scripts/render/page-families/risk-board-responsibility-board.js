function createRiskResponsibilityBoardRenderer(ctx = {}, C = ctx.colors()) {
  ...
  return function drawRiskResponsibilityBoard(slide, s, items = [], board) {
    ...
    addLabel(slide, s.loopLabel || '问题 · 责任 · 动作', { x:board.x+0.28, y:board.y+0.28, w:3.20, h:0.12, fontSize:6.0, color:C.muted, charSpace:0 });
    ...
    addText(slide, s.centerTitle || '责任分工', { x:cx-0.46, y:cy-0.11, w:0.92, h:0.13, fontSize:8.8, bold:true, color:C.white, align:'center', fit:'shrink' });
    addLabel(slide, s.centerLabel || '可追踪', { x:cx-0.56, y:cy+0.10, w:1.12, h:0.08, fontSize:4.7, color:'64748B', align:'center', charSpace:0 });

    const slots = [
      { x:leftX, y:topY, color:C.accent, defaultTitle:'定责' },
      { x:rightX, y:topY, color:C.cyan, defaultTitle:'处置' },
      { x:rightX, y:bottomY, color:C.violet, defaultTitle:'留痕' },
      { x:leftX, y:bottomY, color:'94A3B8', defaultTitle:'复盘' }
    ];
    slots.forEach((slot, i) => {
      ...
      addText(slide, it.owner || it.role || it.accountable || ['责任人', '处置人', '记录人', '复盘人'][i], {
        x:slot.x+cardW-0.94, y:slot.y+0.17, w:0.72, h:0.14, fontSize:8.2, color:slot.color, align:'right', fit:'shrink'
      });
      addText(slide, itemBody(it, ['明确责任与边界。', '推进处置动作。', '沉淀处理记录。', '更新治理机制。'][i]), {
