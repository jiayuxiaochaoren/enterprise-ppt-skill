function createSaasWorkflowField(ctx = {}) {
  ...
  function drawWorkflowField(slide, s, caps, opts = {}) {
    const stage = { x:3.86, y:opts.y || 2.06, w:4.66, h:opts.h || 4.12 };
    ...
    const capSlots = [
      ...
      { x:stage.x+2.50, y:stage.y+Math.max(2.30, stage.h-1.34), anchor:[cx+0.72, cy+0.54], color:C.violet },
      { x:stage.x+0.34, y:stage.y+Math.max(2.30, stage.h-1.34), anchor:[cx-0.72, cy+0.54], color:'94A3B8' }
    ];
    caps.forEach((cap, i) => {
