function createBrandWorldBoardRenderer(ctx = {}) {
  ...
  function drawBrandWorldBoard(slide, board, groups = {}) {
    ...
    [
      { label:'品牌信号', items:groups.drivers, color:C.accent, fallback:'品牌主张' },
      { label:'经营动作', items:groups.actions, color:C.cyan, fallback:'经营动作' },
      { label:'业务证明', items:groups.outcomes, color:C.violet, fallback:'业务结果' }
    ].forEach((group, i) => {
      ...
      group.items.slice(0, 3).forEach((item, j) => {
        ...
        ctx.addText(slide, ctx.itemTitle(item, `${group.fallback}${j + 1}`), {
          ...
          fontSize:6.8, bold:j === 0, color:j === 0 ? C.white : C.text, fit:'shrink', align:'center', valign:'mid', breakLine:true
        });
      });
    });
  }
