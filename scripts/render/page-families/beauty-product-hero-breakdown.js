...
function createBeautyProductHeroBreakdown(ctx = {}) {
  ...
  function drawProductHeroBreakdown(slide, s, items) {
    ...
    (breakdown.length ? breakdown : [{ title:'卖点', body:'客户能直接理解。' }, { title:'场景', body:'对应明确使用对象。' }, { title:'证据', body:'配合数据或案例证明。' }]).slice(0,3).forEach((it,i)=>{
      ...
      addRect(slide, x, 6.00, w, 0.72, panelFill(), C.line, {
        fill:{color:panelFill(), transparency:0},
        line:{color:i===0?accent:C.line, transparency:i===0?20:16, width:0.40}
      });
      addNumber(slide, String(i+1).padStart(2,'0'), {
        x:x+0.20, y:6.26, w:0.30, h:0.10, fontSize:6.8, color:accent
      });
      addText(slide, itemTitle(it, `卖点 ${i+1}`), {
        x:x+0.64, y:6.17, w:0.98, h:0.15, fontSize:8.2, bold:true, color:C.text, fit:'shrink'
      });
      addText(slide, itemBody(it), {
        x:x+1.74, y:6.13, w:w-1.98, h:0.32, fontSize:6.9, color:C.body, fit:'shrink', breakLine:true, valign:'mid'
      });
    });
  }
