function createValueCreationProcessMapRenderer(ctx = {}, deps = {}) {
  ...
  return function valueCreationProcessMapSlide(slide, plan, s, idx) {
    const C = ctx.colors();
    const generalOperations = plan && plan.industry === 'general-operations';
    drawLightPageHeader(slide, {
      kicker:generalOperations ? '经营价值流程' : 'VALUE CREATION PROCESS',
      title:s.title || '价值创造链路',
      titleW:6.0,
      subtitle:s.claim || s.subtitle || (generalOperations
        ? '把资源、协同、交付和结果放在一条可追踪经营流向上。'
        : '把投入、活动、产出和结果放在一条可验证流向上。'),
      subtitleW:6.8,
      idx
    });
    const lanes = [
      { label:generalOperations ? '资源输入' : 'INPUT', title:s.leftTitle || (generalOperations ? '经营资源' : '关键投入'), items:s.drivers || s.inputs || [], color:C.accent },
      { label:generalOperations ? '协同动作' : 'ACTIVITY', title:s.centerTitle || (generalOperations ? '流程协同' : '经营动作'), items:s.actions || s.capabilities || [], color:C.cyan },
      { label:generalOperations ? '阶段产出' : 'OUTPUT', title:s.outputTitle || (generalOperations ? '交付结果' : '直接产出'), items:s.outputs || s.outputItems || (s.actions || []).slice(0, 3), color:C.violet },
      { label:generalOperations ? '结果回报' : 'OUTCOME', title:s.rightTitle || (generalOperations ? '经营回报' : '长期结果'), items:s.outcomes || s.results || [], color:C.muted }
    ];
    ...
    lanes.forEach((lane, i) => {
      ...
      ctx.addLabel(slide, lane.label, {
        x:x + 0.22, y:board.y + 0.30, w:1.04, h:0.10,
        fontSize:6.8, color:lane.color, charSpace:generalOperations ? 0 : 0.7
      });
    ...
    ctx.addLabel(slide, generalOperations ? '经营说明' : 'PROOF NOTE', {
      x:1.16, y:6.38, w:1.02, h:0.10, fontSize:6.8, color:C.accent, charSpace:generalOperations ? 0 : 0.7
    });
    ctx.addText(slide, s.note || (generalOperations
      ? '投入、协同、交付和结果必须能沿链路回看。'
      : '价值流向必须能说明投入如何变成经营结果。'), { x:2.38, y:6.36, w:7.06, h:0.11, fontSize:7.6, color:C.body, fit:'shrink' });
