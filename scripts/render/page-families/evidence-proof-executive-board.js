...
function createExecutiveProofBoardRenderer(ctx = {}) {
  ...
  return function executiveProofBoard(slide, plan, s, idx) {
    ...
    const contentH = Math.max(3.48, 6.02 - contentY);
    const decision = { x:0.92, y:contentY, w:3.22, h:contentH };
    ...
    const board = { x:4.72, y:contentY, w:6.74, h:contentH };
    const slotH = Math.max(1.36, Math.min(1.56, (contentH - 0.86) / 2));
    const slotGapY = Math.max(0.36, contentH - (slotH * 2));
    const slots = [
      { x:board.x, y:board.y, color:C.accent },
      { x:board.x+3.48, y:board.y, color:C.cyan },
      { x:board.x, y:board.y+slotH+slotGapY, color:C.risk },
      { x:board.x+3.48, y:board.y+slotH+slotGapY, color:C.violet }
    ];
    slots.forEach((slot, i) => {
      const item = items[i] || {};
      addRect(slide, slot.x, slot.y, 3.02, slotH, panelFill(), i===0 ? slot.color : C.line, { fill:{color:panelFill(), transparency:0}, line:{color:i===0?slot.color:C.line, transparency:i===0?20:16, width:0.42} });
