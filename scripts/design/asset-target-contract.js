...
function slotTargetForSlide(s = {}, roleText = '') {
  if (String(s.type || '').toLowerCase() === 'cover' && /brand-product|brand-system|product|hero-panel|right-panel/i.test(roleText)) {
    return {
      role: 'showcase',
      slot: { w:7.58, h:7.5 },
      fitPolicy: 'cover',
      targetSource: 'renderer-slot:cover-showcase-right-panel'
    };
  }
...
function assetTargetContract(plan = {}, s = {}, role = '', opts = {}) {
  ...
  const roleText = [
    ...
    visual.targetUse || '',
    s.type || '',
    plan.coverStyle || plan.cover_style || '',
    s.coverStyle || s.cover_style || '',
    s.layoutVariant || '',
    s.variant || '',
    s.proof && s.proof.id
  ].filter(Boolean).join(' ').toLowerCase();
