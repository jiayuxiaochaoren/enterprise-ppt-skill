...
function renderScorecard(ctx, spec = {}, opts = {}) {
  ...
  const cols = values.length <= 3 ? values.length : 3;
  const rows = Math.ceil(values.length / cols);
  const gap = 0.14;
  const cellW = (plot.w - gap * Math.max(0, cols - 1)) / Math.max(1, cols);
  const cellH = Math.min(0.86, (plot.h - gap * Math.max(0, rows - 1)) / Math.max(1, rows));
  values.forEach((value, i) => {
    const cx = plot.x + (i % cols) * (cellW + gap);
    const cy = plot.y + Math.floor(i / cols) * (cellH + gap);
