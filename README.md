...
这个项目可以作为 Node.js 脚本链路运行，也可以配合任意能产出结构化抽取结果的模型或人工流程使用。在 Codex 里体验最佳：Codex 能读取本地材料、按 skill 规则推进澄清、执行素材决策、调用生成脚本、检查预览图，再根据反馈持续修版。需要主视觉但材料没有合适图片时，skill 也支持在用户选择后生成图片并绑定到 PPT 版面中。
...
也可以在 Codex 之外使用：外部模型或人工流程只要提供 `material-extraction.json`、`model-results` 或符合 deck plan 结构的 JSON，就能继续走 `material_to_deck_plan`、资产决策、PPTX 生成和 QA 链路。
...
- 当版面需要主视觉而材料没有合适图片时，链路会要求用户选择补充图片、生成图片或改用无图结构页；在 Codex 中这个决策可以由 agent 直接引导完成。
