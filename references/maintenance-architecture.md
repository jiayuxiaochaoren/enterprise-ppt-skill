...
- Stage component coverage is a policy, not a flat all-or-nothing list. Use `coveragePolicy.requiredAll`, `coveragePolicy.requiredAny`, `coveragePolicy.optional`, and `coveragePolicy.minHits` when a stage needs stricter semantics; legacy `components` are interpreted as `requiredAny` with `minHits: 1` for compatibility.
- Definition-level registry QA lives in `scripts/design/industry-evidence-chain-registry-audit.js`. Run it through `scripts/test_industry_evidence_registry.js` when adding or changing chain definitions so stage positions, explicit coverage policies, evidence contracts, render paths, and avoidComponents stay valid.
...
- Component evidence contracts live in `scripts/design/component-evidence-contracts.js`; render capability data imports those data requirements. Update this registry when an existing industry component such as `equipment-nameplate`, `quality-scorecard`, `service-blueprint-lane`, `prototype-frame`, or `permission-audit-tag` becomes executable.
- Component render paths live in `scripts/render/component-render-path-registry.js`. Use `native`, `overlay`, or `suppressed-by-policy` there instead of inferring renderability from native ownership alone; `source-note` is overlay/suppressed by visible-source policy, not native evidence.
...
- `npm run audit:industry-chain -- --plan <deck-plan.json> [--render-meta <pptx.render-meta.json>]` prints each slide's chain stage, coverage status, policy hits, planned components, consumed components, and blocking gap.
- Regression samples live in `examples/industry-evidence-chain/` and are covered by the `scripts/test_industry_evidence_*.js` suite. These fixtures are not page families; they verify normalized slide/componentPlan/render-meta contracts across P0 and extended industries.
