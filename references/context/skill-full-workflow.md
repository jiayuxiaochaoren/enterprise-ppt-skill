...
- 不在用户可见 PPT 里写“示例、测试稿、验收稿、占位、待补充、材料显示、企业 PDF 口径、正式交付前建议、图册页优先”等制作痕迹。缺失事实放在交付说明或 speaker notes 语境里处理。
...
4. 诊断页、数据页、方案页和价值页必须尽量形成 `现状 -> 影响/差距 -> 原因 -> 动作 -> 指标/结果` 的商业逻辑链；材料没有依据时宁可留空或列入交付说明，不编造。
5. 数据页不能只摆大数字或数字卡片。优先使用同比/环比、目标-当前对比、根因矩阵、漏斗、旅程断点、资源热力、改善前后对比、里程碑、趋势线、散点/气泡矩阵、瀑布桥或 scorecard。
6. 默认 8-12 页，除非用户指定页数。
7. 能用图形结构表达的，不用长段落。
8. 缺少数据时，不在 PPT 里放“待补充”；在交付说明里列“需用户补充”。
9. 真实生成 PPTX 时，必须运行 `scripts/validate_pptx.js`；有条件时用 `--preview-dir` 导出预览图。
10. 面向外部客户、官网、展会、投标或招商时，必须先过商用可外发门槛：品牌身份、事实证据、素材授权、客户/军工/资质合规、联系方式和页码收尾都必须可解释。

## Mandatory Workflow

### 1. Source Extraction

当用户上传一组真实材料（PDF/Word/PPT/Excel/Markdown/截图/图片目录）时，必须优先走混合架构，而不是直接手写 deck plan：

```text
用户材料
  ↓
scripts/material_ingest.js 生成 material-bundle.json
  ↓
scripts/material_orchestration_prompt.js 生成分阶段模型编排提示词
  ↓
模型/脚本分五个检查点完成：来源审计 → 叙事架构 → 用户澄清闸门 → 结构化抽取 → 模型审稿
  ↓
scripts/material_to_deck_plan.js 编译成结构化 deck plan
  ↓
scripts/generate_pptx.js 确定性生成 PPTX
  ↓
validate + visual_qa + contact sheet
```

这条链路的设计原则：**模型负责理解材料，脚本负责稳定生成**。不要让模型每次手写页面坐标，也不要让纯规则假装理解复杂材料。

模型不能把“事实抽取、故事规划、图片用途、风险审稿”塞进同一轮大 prompt。除非材料极少且低风险，否则必须使用分阶段编排：

- **Stage 1 / Source Audit**：只做材料来源审计，区分原始事实、上一版 PPT 二次加工文案、制作备注、视觉证据和外发风险。
- **Stage 2 / Story Architecture**：只决定 deck 类型、行业、受众目标、章节顺序、proof object 计划、行业 deep dive、推荐目录、缺失输入和禁重复规则。
- **Stage 3 / Clarification Gate**：用 `scripts/material_clarification_gate.js` 把 Stage 1/2 的缺失输入变成可问用户的问题；若出现 `needs_user_input`，先问用户并让用户选择补充、跳过、脱敏、保守表达或改为内审稿。
- **Stage 4 / Structured Extraction**：只输出 `material-extraction/v1`，事实必须引用 Stage 1 可用来源，并服从 Stage 2 章节、proof object、商业逻辑链、数据组件计划和 Stage 3 用户选择。
- **Stage 5 / Model Critic**：编译前审稿，检查来源污染、内容重复、真实图片证据、生成图滥用、结束页语义和外发风险；未通过时先修抽取结果，不进入 deck plan。

第一步先摄取材料：

```bash
node scripts/material_ingest.js <材料文件或目录...> --out out/material-bundle.json
node scripts/material_orchestration_prompt.js \
  --bundle out/material-bundle.json \
  --out-dir out/model-orchestration
```

然后由执行 skill 的 agent/当前模型依次读取：

```text
out/model-orchestration/01-source-audit.prompt.md
out/model-orchestration/02-story-architecture.prompt.md
out/model-orchestration/03-clarification-gate.md
out/model-orchestration/04-extraction.prompt.md
out/model-orchestration/05-critic.prompt.md
```

每一轮输出都必须保存为独立 JSON：

```bash
out/model-orchestration/source-audit.json
out/model-orchestration/story-architecture.json
out/model-orchestration/clarification-gate.json
out/material-extraction.json
out/model-orchestration/model-critic.json
```

Stage 3 必须在 Stage 2 后运行。如果输出 `status: needs_user_input`，先向用户展示问题和选项；用户回答后保存为 `clarification-answers.json` 并重新解析 gate：

```bash
node scripts/material_clarification_gate.js \
  --bundle out/material-bundle.json \
  --source-audit out/model-orchestration/source-audit.json \
  --story-plan out/model-orchestration/story-architecture.json \
  --out out/model-orchestration/clarification-gate.json

node scripts/material_clarification_gate.js \
  --bundle out/material-bundle.json \
  --source-audit out/model-orchestration/source-audit.json \
  --story-plan out/model-orchestration/story-architecture.json \
  --answers out/model-orchestration/clarification-answers.json \
  --out out/model-orchestration/clarification-gate.json
```

Clarification gate 的问题必须短、可选择、可执行，通常覆盖：

- 联系人/官网/地址/二维码是否可外发展示。
- 客户案例、项目名、客户 logo、敏感行业案例是否有授权，是否需要脱敏。
- 资质、证书、专利、荣誉是否有编号、年份、有效期和截图。
- 数据页是否有真实指标、口径、目标值或前后对比。
- 现场图、产品图、截图、证书图等视觉证据是否可用。
- 受众和决策目标是否明确。

如果用户选择不补充，后续抽取必须使用 gate 的 fallback：跳过相关页面、脱敏、定性表达、生成内审稿或从可见 PPT 中移除相关承诺。不能把缺失内容写成事实。

Stage 4 需要把前三个检查点 JSON 嵌入后再生成最终提示词：
...
node scripts/material_orchestration_prompt.js \
  ...
  --source-audit out/model-orchestration/source-audit.json \
  --story-plan out/model-orchestration/story-architecture.json \
  --clarifications out/model-orchestration/clarification-gate.json \
  --out out/model-orchestration/04-extraction.prompt.md
```

Stage 5 审稿通过，或所有 revision instructions 已处理后，才能编译 deck plan。模型抽取结果必须保存为：
...
```bash
node scripts/material_to_deck_plan.js \
  --bundle out/material-bundle.json \
  --model-json out/material-extraction.json \
  --out out/deck-plan.json
```

`scripts/material_model_prompt.js` 只允许作为极简材料的兼容入口。凡是公司介绍、对外交付、材料来源混杂、含图片/证书/客户案例、或用户反馈“理解/汇总/重复/设计感不足”的任务，都必须走 `material_orchestration_prompt.js` 和 `material_clarification_gate.js`。

模型抽取只做结构化理解，不做版式坐标。抽取字段包括：
...
```json
{
  ...
  "date": "材料中明确给出的日期，可为空",
  "contacts": ["材料中明确给出的联系人、电话、邮箱、官网、地址或二维码说明，可为空"],
  "decision_goal": "争取预算 / 推动合作 / 项目立项 / 管理对齐 / 销售转化",
  "clarifications": [
    {"id": "用户澄清问题 id", "choice": "用户选择", "value": "用户补充事实或空", "effect": "对目录/页面/风险的影响"}
  ],
  "deck_art_direction": {
    "tone": "premium-industrial-editorial / executive-boardroom / calm-clinical / capital-governance / product-platform",
    "palette": "visual-system palette id",
    "semantic_color_roles": {
      "brand": "品牌/行业身份",
      "evidence": "证据、图片 caption、source note",
      "risk": "问题、约束、授权、合规",
      "action": "路径、下一步、决策",
      "data": "指标、对比、价值信号",
      "neutral": "背景、正文、细规则"
    },
    "layout_diversity_rules": ["避免相邻页重复同一种卡片/背景/微组件"],
    "rhythm_map": [
      {
        "slideId": "claim id",
  ...
  "commercial_risks": ["客户名授权、军工脱敏、证书有效期、图片来源、字体授权等外发风险"],
  "missing_info": ["需要用户补充但不放进可见 PPT 的信息"]
}
```

如果信息不足，先进入 clarification gate；用户选择不补充或仍缺依据时，模型抽取层必须写入 `missing_info`，编译器会把它保留在 `materialIntelligence.missingInfo`；不要把“待补充”写进可见 PPT 页面。

### 1.1 Commercial Readiness Gate

当目标是“商用、对外、客户可发、官网/展会/投标/招商”时，生成前必须做一次商用审查；不达标则在交付说明中标明“可作沟通稿/内审稿，不建议直接外发”：

- **品牌身份**：封面和结尾必须出现明确组织名；有 logo、品牌色、官网、联系人、二维码或地址时优先使用。没有联系方式时不能编造，也不能把“潜在客户高层 / 采购负责人 / 产业合作伙伴 / 日期”这类内部定位当作外发封面元信息。
- **事实硬度**：公司介绍、能力介绍、资质页和案例页必须优先使用可核验事实：年份、厂区/车间规模、设备数量、证书名称/年份/编号/有效期、项目年份、交付范围、关键参数、验收或运行结果。无法核验的数字不写成定论。
- **案例合规**：客户名称、客户 logo、军工/轨交/船舶等敏感案例、现场照片、设备参数和项目名称，只有在材料明确公开或用户确认可用时才对外展示；否则脱敏、行业化或放入内审版。
- **素材授权**：图片、视频、logo、证书截图和字体必须有来源与授权状态；真实客户、真实现场和真实数据截图不能用生图或图库图冒充。
- **官方口吻**：可见文案必须像企业官方材料，不像制作备注。禁止出现“材料显示”“企业 PDF 简介口径”“该页用于”“正式交付前建议”“图册页优先展示”“适合某类材料”等元叙事。
- **收尾完整**：页码顺序、页脚、联系方式、下一步动作、版本日期和导出格式必须一致。公司介绍/宣传册可以用“谢谢观看/期待交流”结束；客户提案、方案评审、投委会材料才使用“下一步/决策/行动项”结束。只有“THANK YOU/谢谢观看”且没有联系人或下一步动作，不算正式外发终页，必须在交付说明列为缺口。

制造业/装备企业能力介绍的额外门槛：

- 封面优先用真实厂区、产线、产品或设备图建立第一眼可信度；不要只用抽象图形和英文标签。
- 传统制造企业宣传 PPT 不能按泛咨询 deck 处理。默认结构应包含：封面、目录、公司介绍、发展/规模、产品与工艺、制造/质量能力、项目领域或案例、资质荣誉、合作/联系方式或致谢页。若材料没有公司照片、厂区图或产品图，必须优先用材料中的图册/页面截图裁切；只有抽象氛围图不足以支撑“公司介绍”页。
- 外发版封面标题优先使用公司全称；“能力介绍/宣传册/公司介绍”可放副标题或文件名，不宜在每页页脚重复。页脚默认只放公司名或品牌名，不显示受众、日期、内部评审对象。
- 产品谱系不能过度软件化。只有材料明确提供平台、数据底座、运营看板等能力时才使用相关说法；否则聚焦工艺、输送、涂装、控制、安装调试和维护服务。
- 资质页必须把荣誉、信用、研发平台、专利/软著等分清；数量口径不一致时不写死，放交付说明。
- 客户项目页至少给出行业分组 + 典型项目类型；正式外发版优先补 2-3 个深案例，而不是只罗列大客户名称。
...
- `scripts/design-system.js` 是唯一的设计决策层，负责把 deck plan + slide 内容解析成 `palette / role / mode / imageRole / pageFamily / imagePath / deckArtDirection / compositionPlan`。
- LLM 只能输出设计导演意图：`deck_art_direction`、`themeIntent`、`accentRole`、`layoutEnergy`、`visualDensity`、`rhythmTransition`。脚本负责把它们稳定落地；不要让 LLM 手写坐标或每页自由换色。
- 结构化数据必须先进入页面构图语法，不能直接从 `claim/proof_object/images` 跳到固定 renderer。每页必须具备 `compositionPlan`，至少包含 `composition`、`layoutPlan`、`backgroundTone`、`themeCoverage`、`themeIntent`、`accentRole`、`layoutEnergy`、`visualDensity`、`rhythmTransition`、`primaryZone`、`secondaryZone`、`proofZone`、`primaryColorUse`、`semanticColorRoles`、`imageTreatment`、`microComponents`、`rhythmRole`。
- `compositionPlan` 是结构化数据和视觉执行之间的契约：它决定主图/副证据/指标/流程/风险/联系方式分别落在哪个区块，决定浅色页是不是 `tinted-paper`、强节奏页是不是 `accent-wash` 或 `dark-stage`，也决定图片是证据框、图册、showcase 还是氛围背景。
- 设计丰富度来自页面角色和证据语法，不来自装饰堆叠。每套 8-12 页 deck 至少应有 opening/navigation/diagnosis 或 setup/proof/system/value 或 risk/closing 等不同 theme intent。
- 语义色必须有职责：`brand` 承担身份和结构，`evidence` 承担证据和图片关系，`risk` 承担问题/约束，`action` 承担路径和决策，`data` 承担指标和价值信号，`neutral` 承担阅读背景。
- 主色调必须有预算，不能只在封面和结束页出现。内页至少要有 2-4 个主色承载点，如顶部线、页码、caption bar、深色侧栏、指标高亮、卡片边线、底部轨道或侧向色场。浅色页默认使用行业色温的 `tinted-paper`，不要退成纯白草稿页。
- Deck 级节奏由 `normalizeDeckPlan`/`applyDeckRhythm` 规划：连续浅色页需要 accent anchor，章节/结束/关键证据页要承担深浅节奏变化，封面和封底应有视觉呼应。若 LLM 没给完整 rhythm map，脚本必须按页面类型、proof object 和语义信号推断。
- 缺图补图必须在 `scripts/design-system.js` 的架构层决策：页面族/recipe 判定该页需要视觉资产、确认没有真实素材、确认生成用途安全后，才能生成 `assetGeneration` 和 `generatedAssetPrompt`。执行层只消费这个决策，不临时猜测要不要生图。
- `scripts/generate_pptx.js` 只负责绘制页面族和消费 `compositionPlan`，不应再直接新增行业配色、图片用途、默认素材或“某行业某页要不要放图”的零散判断。
- `scripts/generate_pptx.js` 里的页面绘制应优先复用微组件，而不是在每个 renderer 里重复手写：`TopRule`、`PageNumber`、`TintedBackground`、`WatermarkCircle`、`EvidenceImageFrame`、`CaptionBar`、`MetricStrip`、`DarkSidebar`、`ProcessRail`、`IndustryTag`、`SourceNote`、`ContactBlock`、`CertificateWall`、`EquipmentNameplate`。
...
- 中文 PPT 的可见标签、kicker、系统标签和微型说明默认使用中文；品牌名、产品名、URL、邮箱、股票代码和 API/OEE/IRR/SKU 等必要缩写保留原文并使用 `Avenir Next`。
- 数字：`DIN Alternate`，用于页码、序号和指标。
- 内置 typography：封面标题、页标题、卡片标题、正文、caption、kicker、页码都有统一 token；局部微调不能破坏最小可读字号。
- 字号不是唯一可读性门槛。任何正文/caption 同时受 `fontSize + 文本框宽度 + 文本长度` 约束；证据图册、设备细节、案例图片、截图说明等页面不得把中文说明挤进低于约 1.15 英寸的窄列，放不下时要改为上下栈、caption bar、source note 或删减文案。
...
- 内置 category reference sampler：当材料暴露行业、页面角色、图表/图解结构、视觉气质或资料类型时，先查 `out/reference-corpus/meta/slideland-category-reference-samples.json` 的中文/英文分类样本，再把抽象标签转成 theme intent、palette intent、页面族和 QA 约束；索引说明见 `references/slideland-category-reference-sampler.md`。
...
- 内置 commercial logic QA：数据/指标页若没有现状、影响/差距、原因、动作和衡量指标，会被 `metricBusinessLogic` 或 `commercialLogicThin` 标记；这类问题必须回到材料理解和页面组件层处理。
...
- 内置 readability QA：`scripts/visual_qa.js` 不只看最小字号，还会检查窄文本框和字符密度。出现 `unreadableNarrowText` 时必须回到全局组件或页面族调整，不能只修当前页坐标。
- 内置 composition QA：用 `compositionAudit` 检查 `compositionPlanMissing`、`themeCoverageLow`、`tooManyWhitePages`、`accentOnlyAsThinLine`、`missingBrandMotif`、`weakImageTreatment`、`flatPageRhythm`、`compositionTooGeneric`、`adjacentLayoutSimilarity`、`semanticColorMismatch`、`closingLacksWeight`。这些是截图级审美前置门槛，不能只靠 PPTX 结构通过。
...
- 高端定制感来自客户事实和行业证据，不来自更复杂的装饰。连续出现“标题 + 副标题 + 卡片 + 编号 + 装饰圆”的模板节奏时，必须改成行业 proof object：旅程断点、根因图、责任闭环、产品证据、案例对比、资源热力或决策表。
...
- 内容去重是硬门槛：公司概况、能力证明、指标页和相邻图册页不能复用同一组事实或数字。`scripts/material_to_deck_plan.js` 会在公司介绍中消费并过滤重复基础指标页，`scripts/visual_qa.js --plan` 会通过 `contentOverlapAudit` 对相邻页重复事实直接 fail。
- 证据图册页是证据组件，不是图片旁边塞说明。`evidence-board` 中每个辅助证据只能保留短标题 + 一句短 caption；长说明必须进入 source note、讲稿备注或拆成单独分析页。机器 QA 会对超预算 caption 给出 `evidenceCaptionBudget`。
- 传统制造企业公司介绍优先使用 `company-profile-spread`：左侧公司定位和简短事实，右侧真实厂区/车间/产品图，底部可核验指标；不要再用泛咨询 profile card 或把同一组规模数据拆成第二页重复讲。
...
- 结束页优先走 `closing`，并根据内容选择 `editorial-light`、`image-statement`、`decision-board`、`decision-summary`、`thank-you`、`simple-end` 或传统企业 `company-thanks`；感谢页、答疑页、董事会决策页和下一步行动页不能互相硬套。触发规则：标题/变体明确包含“谢谢/感谢/答疑/Q&A”才走感谢页；有 `decision/actions/summary` 且属于提案/评审/董事会材料才走决策或下一步页；公司介绍缺联系人时默认走制造企业封底式 `company-thanks`，并在交付说明提示联系方式缺口。封底必须有明确视觉重心、深浅对比和可读公司名，右侧真实图片不能被高透明浅色遮罩洗白。
- 指标/KPI/同比/利润/百分比材料优先走 `metric-comparison`，但必须把数字接回业务逻辑：现状、差距/影响、原因、动作和衡量指标；制造业 OEE、产线、停机、维修效率材料优先走 `oee-board`，把稼动、性能、良率和维修信号放在同一页。
...
- 若参考 recipe 判断页面需要视觉资产而材料缺图，先运行 `node scripts/deck_asset_decision_gate.js deck-plan.json --out out/asset-gate.json`。如果 gate 返回 `needs_user_input`，必须让用户在“提供素材 / 跳过图片 / 自动生成示意图”之间选择，不能静默落成几何占位图。
- 用户选择自动生成后，再运行 `node scripts/asset_prompt_planner.js deck-plan.json --out out/asset-prompts.json` 生成生图提示词。注意：本地 Node 脚本不会、也不能直接调用 Codex 内置 `imagegen`；执行 Skill 的 agent 必须在对话侧调用 imagegen 生成项目本地 bitmap 资产，再用绑定脚本写回 deck plan。
- `asset_prompt_planner.js` 只能执行 `assetGeneration.status` 为 `required/optional` 的页面；`blocked` 页面必须改用真实素材、脱敏素材或纯结构绘制。
...
如果输出 `blockedCount > 0`，不要绕过：这表示该页缺的是事实证据图、真实截图、客户案例、证书、logo、真实人物或敏感现场，必须要求真实素材/授权、做脱敏版，或改成原生图表/结构页。
...
```json
{
  ...
  "date": "材料中明确给出的日期（可省略）",
  "contacts": ["材料中明确给出的电话、邮箱、官网、地址或二维码说明（可省略）"],
  "footer": "材料中明确给出的页脚（可省略）",
  "commercialReview": {
    "readyForExternalUse": false,
    "openRisks": ["缺少联系方式、客户案例授权、证书有效期、图片来源等外发风险"]
  },
  "showMeta": true,
  "showFooter": true,
...
- 每页 claim 明确，没有空泛标题。
- 没有编造数据、客户、政策、收入、奖项。
- 对外商用版已确认品牌身份、联系方式/下一步动作、客户案例授权、资质口径、图片来源和敏感信息处理；未确认的内容必须在交付说明里列为风险，不装进页面当成事实。
...
- 不出现“示例、测试稿、验收稿、占位、待补充、Lorem、TODO、材料显示、企业 PDF、PDF 简介口径、正式交付前、图册页优先、该页用于、该页只展示、模型抽取”等可见脚手架或制作备注。
- 结束页不能只有空泛感谢；至少要有联系人/官网/二维码/地址/下一步评审动作之一。
